...
class diff_match_patch:
    ...
    def diff_compute(self, text1, text2, checklines, deadline):
        ...
        if i != -1:
            ...
            diffs = [
                (self.DIFF_INSERT, longtext[
                    :i]), (self.DIFF_EQUAL, shorttext), (self.DIFF_INSERT, longtext[
                        i + len(shorttext):])]
    ...
    def diff_cleanupSemantic(self, diffs):
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                ...
                if (
                    lastequality and (
                        len(lastequality) <= max(
                            length_insertions1,
                            length_deletions1)) and (
                        len(lastequality) <= max(
                        length_insertions2,
                        length_deletions2))):
                    # Duplicate record.
                    diffs.insert(
                        equalities[-1], (self.DIFF_DELETE, lastequality))
            ...
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                ...
                if lastequality and (
                    (pre_ins and pre_del and post_ins and post_del) or (
                        (len(lastequality) < self.Diff_EditCost /
                         2) and (
                            pre_ins +
                            pre_del +
                            post_ins +
            ...
            if diffs[pointer][0] == self.DIFF_INSERT:
                ...
                if count_delete + count_insert > 1:
                    if count_delete != 0 and count_insert != 0:
                        ...
                        if commonlength != 0:
                            diffs[pointer] = (
                                diffs[pointer][0], text_insert[-commonlength:] + diffs[pointer][1])
                            text_insert = text_insert[:-commonlength]
                            text_delete = text_delete[:-commonlength]
                    # Delete the offending records and add the merged ones.
                    if count_delete == 0:
        ...
        while pointer < len(diffs) - 1:
            if (diffs[pointer - 1][0] == self.DIFF_EQUAL and
                ...
                if diffs[pointer][1].endswith(diffs[pointer - 1][1]):
                    ...
                    diffs[pointer] = (diffs[pointer][0], diffs[
                                      pointer - 1][1] + diffs[pointer][1][:-len(diffs[pointer - 1][1])])
                    diffs[
                        pointer +
                        1] = (
                        diffs[
                    ...
                    diffs[pointer] = (diffs[pointer][0], diffs[pointer][1][
                                      len(diffs[pointer + 1][1]):] + diffs[pointer + 1][1])
                    del diffs[pointer + 1]
                    changes = True
            pointer += 1
    ...
    def diff_prettyHtml(self, diffs):
        ...
        for (op, data) in diffs:
            ...
            if op == self.DIFF_INSERT:
                html.append(
                    "<ins style=\"background:#e6ffe6;\">%s</ins>" %
                    text)
            elif op == self.DIFF_DELETE:
                html.append(
                    "<del style=\"background:#ffe6e6;\">%s</del>" %
                    text)
    ...
    def diff_fromDelta(self, text1, delta):
        ...
        for token in tokens:
            ...
            if token[0] == "+":
                ...
                if n < 0:
                    raise ValueError(
                        "Negative number in diff_fromDelta: " +
                        param)
    ...
    def patch_addContext(self, patch, text):
        ...
        while (
            text.find(pattern) != text.rfind(pattern) and (
                self.Match_MaxBits == 0 or len(pattern) < self.Match_MaxBits -
                self.Patch_Margin -
                self.Patch_Margin)):
            ...
            if len(text1) > self.Match_MaxBits:
                ...
                if start_loc != -1:
                    end_loc = self.match_main(
                        text, text1[-self.Match_MaxBits:], expected_loc + len(text1) - self.Match_MaxBits)
                        ...
                        for (op, data) in patch.diffs:
                            ...
                            if op == self.DIFF_INSERT:  # Insertion
                                text = text[:start_loc + index2] + \
                                    data + text[start_loc + index2:]
                            elif op == self.DIFF_DELETE:  # Deletion
                                text = text[
                                    :start_loc + index2] + text[start_loc + self.diff_xIndex(diffs, index1 + len(data)):]
                            if op != self.DIFF_DELETE:
                                index1 += len(data)
        ...
        for x in xrange(len(patches)):
            ...
            while len(bigpatch.diffs) != 0:
                ...
                if postcontext:
                    ...
                    if len(
                            patch.diffs) != 0 and patch.diffs[-1][0] == self.DIFF_EQUAL:
                        patch.diffs[-1] = (self.DIFF_EQUAL,
